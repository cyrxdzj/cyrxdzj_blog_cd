...
在竞赛中，常用的单哈希（如模 $10^9+7$）存在被卡的风险——出题人可以构造数据使得大量不同字符串哈希值相同，导致程序出错或超时。双哈希就是为了解决这一问题而设计的。

### 双哈希的核心思想

双哈希使用**两个不同的哈希函数**对同一字符串分别计算哈希值，只有当两个哈希值都相等时才认为两个字符串相等。这样，冲突概率约为两个哈希函数冲突概率的乘积，可低至 $10^{-18}$ 量级，在竞赛中可以认为是零冲突。
...
Hash operator+(Hash a,char b)//向末尾添加字符串
{
    Hash res;
    res.len=a.len+1;
    for(int i=0;i<MODCNT;i++)
...
bool operator<(Hash a,Hash b)//定义小于运算以用于set存储
{
    if(a.len>b.len)
    {
        return false;
...
本题可以存储每个字符串的哈希前缀和和哈希后缀和，然后枚举“是哪个位置不同（设位置为j）”，将j-1的哈希前缀和和j+1的哈希后缀和用pair存储后放进unordered_map里统计（map会超时）。
