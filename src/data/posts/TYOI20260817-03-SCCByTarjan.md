...
- `low[u]`：在 $u$ 的子树中能够回溯到的最早的已经在栈中的节点的DFS序。

**关键引理**：在一个强连通分量中，`dfn` 值最小的节点（即DFS最先访问到的那个节点）是SCC的“根”。当DFS回溯到该根节点时，如果 `dfn[u] == low[u]`，那么栈中从栈顶到 $u$ 的所有节点恰好构成一个强连通分量。

**证明**：`dfn[u] == low[u]` 说明从 $u$ 出发，无法走到任何DFS序比 $u$ 更早的节点。而栈中在 $u$ 之后的节点（比 $u$ 晚入栈），都是DFS过程中从 $u$ 的子树中遍历到的。由于无法往比自己更早的节点走，因此栈中 $u$ 以上的节点均无法连接到 $u$ 以外的节点，与 $u$ 共同构成一个强连通分量。
...
void dfs(int u)
{
    ...
    for (int i = head[u]; i; i = edge[i].next)
    {
        ...
        else if (instk[v])          // 子节点在栈中（尚未确定所属SCC）
        {
...
算法使用的栈和标记数组的额外空间为 $O(n)$；若计入图的邻接表存储，则总空间复杂度为 $O(n + m)$。
