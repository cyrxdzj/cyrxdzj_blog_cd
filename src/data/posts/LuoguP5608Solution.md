...
| 变量名 | 含义 | 左段 | 右段 | 总段 |
| :--- | :--- | :--- | :--- | :--- |
| (仅为示例，不在代码中维护。) | 算式 | $1 \times 2$ |$3+4$| $1 \times 2 \times 3 + 4$ |
| `len` | 区间长度。可以通过 $r - l + 1$ 计算出来，辅助判断极长连乘段。 | $2$ | $2$ | $4$ |
| `rop` | 这一段右侧的运算符的对应代号。$0$ 为 $+$，$1$ 为 $\times$。 | $1$ | 不必考虑，无论存储为多少都没问题。 | 同左。 |
| `llen` | 左端极长连乘段的长度 | $2$ | $1$ | $3$ |
| `rlen` | 右端极长连乘段的长度 | $1$ | $1$ | $1$ |
| `sum` | 最终答案 | $2$ | $7$ | $10$ |
| `lsum` | 左端极长连乘段结果 | $2$ | $3$ | $6$ |
| `rsum` | 右端极长连乘段结果 | $2$ | $4$ | $4$ |
...
| 变量名 | 含义 | 左段 | 右段 | 总段 |
| :--- | :--- | :---: | :---: | :---: |
| (仅为示例，不在代码中维护。) | 算式 | $1\times2$ | $3+4$ | $1\times2\times3+4$ |
| `addsum` | 如果这一段都是加法，结果为多少？ | $3$ | $7$ | $10$ |
| `mulsum` | 如果这一段都是乘法，结果为多少？ | $2$ | $12$ | $24$ |
| `lval` | 最左边的值 | $1$ | $3$ | $1$ |
| `rval` | 最右边的值 | $2$ | $4$ | $4$ |
