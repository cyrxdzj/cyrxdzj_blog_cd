...
以Luogu P2495 【SDOI2011】 消耗战为例：给定一棵 $n$ 个节点的树，边有边权，每次给出 $k$ 个关键点，要求切断一些边使得根节点（1号节点）与所有关键点都不连通，求最小代价。

如果每次都在整棵树上DP，单次复杂度为 $O(n)$。但题目有 $m$ 次询问，总复杂度 $O(nm)$ 显然过高。

注意到每次只与 $k$ 个关键点有关，我们可以构建一棵只包含这 $k$ 个关键点以及它们两两之间LCA的虚树，然后在虚树上进行DP。这样单次复杂度降为 $O(k \log k)$（排序+建虚树+DP）。
...
1. 将所有关键点按DFS序从小到大排序；
2. 将根节点（1号节点）入栈，清空其邻接表；
3. 依次处理每个关键点 $h[i]$：
   - 计算 $l = \text{LCA}(h[i], \text{栈顶节点})$；
   - 如果 $l$ 等于栈顶节点，说明 $h[i]$ 在当前栈所存的链上，直接将 $h[i]$ 入栈；
   - 否则，需要从栈中弹出节点，直到找到合适的位置连接。具体分三种情况处理；
4. 处理完所有关键点后，将栈中剩余节点依次连边。
...
flowchart TD
    ...
    H --> D3[下一个关键点]
    D3 --> D
    G -->|否| I{"dfn[l]小于dfn[次大节点]吗"}
    I -->|是| J[连边次大节点到栈顶\n弹出栈顶]
    J --> I
    I -->|否| K{"dfn[l]大于dfn[次大节点]吗"}
    K -->|是| L[连边l到栈顶，\n弹出栈顶，l入栈]
    K -->|否| M[连边l到栈顶\n弹出栈顶]
    L --> N["此关键点入栈"]
    M --> N
...
1. 从栈中弹出节点，直到次大节点的DFS序不大于LCA的DFS序；
2. 每弹出一个节点，将其与新的栈顶连边（这条边属于虚树）；
3. 判断LCA是否等于次大节点：
   - 若LCA不等于次大节点，说明LCA是第一次入栈，需要将其入栈；
   - 若LCA等于次大节点，直接将栈顶弹出即可。
...
虚树构建完成后，就可以在上面进行动态规划了。以消耗战为例：

设 $dp[u]$ 表示在以 $u$ 为根的子树中，切断所有关键叶子节点的最小代价。

$$dp[u] = \sum_{v \in \text{children}(u)} \min(w(u,v), dp[v])$$

其中 $w(u,v)$ 是边 $(u,v)$ 的权值。如果 $v$ 是关键节点，则 $dp[v] = +\infty$（必须切断与父亲的连边）。

### 时间复杂度分析

- 排序关键点：$O(k \log k)$
- 每次LCA查询：$O(\log n)$（倍增法）或 $O(1)$（DFS序+ST表）
- 建虚树：$O(k \log n)$ 或 $O(k)$
- 虚树上DP：$O(k)$

总复杂度：$O(k \log k + k \log n)$，远优于在整棵树上DP的 $O(n)$。
